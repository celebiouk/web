...
export async function POST(request: Request) {
  try {
    ...
    let requestedCountryCode: string | null = null;

    try {
      const body = await request.json();
      requestedCountryCode = normalizeCountryCode((body as { countryCode?: string | null })?.countryCode);
    } catch {
    ...
    const payoutCountryCode = normalizeCountryCode(profile.payout_country_code) || requestedCountryCode || '';
    ...
    if (!profile.payout_country_code) {
      await (supabase.from('profiles') as any)
        .update({ payout_country_code: payoutCountryCode, payout_provider: 'stripe' })
        .eq('id', user.id);
    }
